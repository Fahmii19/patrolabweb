    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('pleton-patrol-area.update', $pleton_patrol->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="patrol_area_id" id="patrol_area_id" required>
                                    ...
                                    @foreach ($patrol_area as $item)
                                        <option value="{{ $item->id }}" {{ old('patrol_area_id', $pleton_patrol->patrol_area_id) == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
