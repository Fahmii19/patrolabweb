                ...
                <div class="row row-cols-1 row-cols-lg-2">
                    <div class="col">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('id_pleton') is-invalid @enderror" name="id_pleton" id="id_pleton">
                                <option value="" selected disabled>--Pilih--</option>
                                @foreach ($pletons as $item)
                                <option value="{{ $item->id }}" {{ old('id_pleton') == $item->id ? 'selected' : '' }}>{{ $item->nama }}</option>
                                @endforeach
                            </select>
                            @error('id_pleton')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('id_guard') is-invalid @enderror" name="id_guard" id="id_guard">
                                <option value="" selected disabled>--Pilih--</option>
                                @foreach ($guards as $item)
                                <option value="{{ $item->id }}" {{ old('id_guard') == $item->id ? 'selected' : '' }}>{{ $item->nama }}</option>
                                @endforeach
                            </select>
                            @error('id_guard')
                            <span class="text-danger d-block">{{ $message }}</span>
