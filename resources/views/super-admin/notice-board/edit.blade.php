    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{route('notice-boards.update', $notice_board->id)}}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row row-cols-1 row-cols-lg-2">
                        <div class="col">
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="area_id" id="area_id" required>
                                    ...
                                    @foreach ($area as $item)
                                        <option value="{{ $item->id }}" {{ old('area_id', $notice_board->area_id) == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="title" value="{{old('title', $notice_board->title)}}" placeholder="Masukkan judul pengumuman" required>
                                @error('code') <span class="text-danger d-block">{{$message}}</span> @enderror
                            </div>
                            <div class="mb-3">
                                <label for="description" class="form-label">Deskripsi <span class="text-danger">*</span></label>
                                <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="description" placeholder="Deskripsi tentang area patrol" rows="4" required>{{ old('description',$notice_board->description) }}</textarea>
