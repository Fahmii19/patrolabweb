                    ...
                    <div class="row row-cols-2">
                        <div class="col">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('area_id') is-invalid @enderror" name="area_id" onchange="get_patrol_area(this.value)" id="selectArea" required>
                                    ...
                                    @foreach ($area as $item)
                                        <option value="{{ $item->id }}" {{ old('area_id', $round->patrol_area->area_id) == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('patrol_area_id') is-invalid @enderror" name="patrol_area_id" id="selectPatrolArea" required>
                                    ...
                                    @foreach ($patrol_area as $item)
                                        <option value="{{ $item->id }}" {{ old('patrol_area_id', $round->patrol_area->id) == $item->id ? 'selected' : '' }}>{{ $item->name }} </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" value="{{ old('name', $round->name) }}" placeholder="Masukkan Nama Rute" required>
                                @error('name') <span class="text-danger d-block">{{$message}}</span> @enderror
                            </div>
                            <div class="mb-3">
                                <label for="status" class="form-label d-block">Status Round</label>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" value="ACTIVED" name="status" id="status" @if($round->status == 'ACTIVED') checked @endif>
